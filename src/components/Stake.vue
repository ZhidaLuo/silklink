...
<div>
  ...
  <el-card class="margining">
    <p>质押治理合约地址： {{ contractAddr }} </p>
    <p> 我的余额: {{ myBalance }} , 质押量: {{ stakedAmount }} </p>
...
export default {
  ...
  methods: {
    ...
    getStaked() {
      this.governor.balances(this.account).then(
        (r) => {
          this.stakedAmount = this.web3.utils.fromWei(r.toString());
        }
      )
    },
    ...
    stake() {
      let amount = this.web3.utils.toWei(this.stakeAmount);
      this.governor.stake(amount, {from: this.account}).then(() => {
        this.getStaked()
        this.getBalance();
        this.getBlockNumber()
      });
